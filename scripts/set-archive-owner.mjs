...
let ownerId = null;
for await (const o of to.items.listPagedIterator({ filter: { type: OWNER }, version: "current", perPage: 100 })) {
  if (val(o.label, "it") === OWNER_LABEL) ownerId = o.id;
}
if (!ownerId) { console.error(`Owner "${OWNER_LABEL}" non trovato`); process.exit(1); }
console.log(`Owner "${OWNER_LABEL}" id: ${ownerId}  [${COMMIT ? "COMMIT" : "DRY-RUN"}]`);
...
function rebuild(b) {
  ...
  if (key === "catalogue_feed") attributes.tabs = (b.attributes.tabs || []).map(rebuild);
...
for await (const p of to.items.listPagedIterator({ filter: { type: IP }, version: "current", nested: true, perPage: 50 })) {
  ...
  console.log(`  ${COMMIT ? "set" : "[dry] setterei"} filter_owner su archivio "${slug}"`);
  if (COMMIT) {
