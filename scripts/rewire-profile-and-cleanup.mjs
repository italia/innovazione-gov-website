...
async function listAll(modelId) {
  const r = [];
  for await (const x of to.items.listPagedIterator({
    filter: { type: modelId },
    version: "current",
    perPage: 100,
  }))
...
console.log(
  `[#2] ri-cablaggio preview profilo  [${COMMIT ? "COMMIT" : "DRY"}]`,
);
...
for await (const p of to.items.listPagedIterator({
  ...
  for (const loc of Object.keys(p.content)) {
    for (const b of p.content[loc]) {
      ...
      if (PREVIEW[title] && tab) {
        const mapped = (tab.attributes.news || [])
          .map((sid) =>
            slugToDistinctId[PREVIEW[title].model].get(storyIdToSlug.get(sid)),
          )
          .filter(Boolean);
        summary.push(
          `${loc}/${title}: ${(tab.attributes.news || []).length} -> ${mapped.length} (${PREVIEW[title].model})`,
        );
      }
    }
  }
  ...
  if (COMMIT) {
    const content = {};
    for (const loc of Object.keys(p.content))
      content[loc] = p.content[loc].map(rebuild);
...
const delClasses = new Set(
  Object.values(PREVIEW).map((p) => classId[p.storyClass]),
);
const targets = storyItems.filter((r) =>
  delClasses.has(val(r.article_classification, "it")),
);
console.log(
  `  story_item da cancellare (Interviste/Interventi/Comunicati): ${targets.length}`,
);
if (COMMIT) {
  let del = 0;
  for (const r of targets) {
    try {
      ...
      console.log(
        `  ! non cancellato ${val(r.slug, "it")}: ${String(e.message).slice(0, 60)}`,
      );
    }
  }
