...
const CATEGORIES = [
  ...
  },
  {
    ...
    source: "interview",
    slug: "interviste",
    ctaAll: "Vedi tutte le interviste",
  },
  {
    key: "interventi",
    label: "Interventi",
    ...
    slug: "interventi",
    ctaAll: "Vedi tutti gli interventi",
  },
  {
    key: "comunicati",
    ...
    ctaAll: "Vedi tutti i comunicati",
  },
];
...
const loc = (f) => {
  const o = {};
  for (const l of LOCALES) o[l] = val(f, l) || "";
  return o;
};
...
const locReq = (f) => {
  const it = val(f, "it") || "";
  const o = {};
  for (const l of LOCALES) o[l] = val(f, l) || it;
  return o;
};
...
async function loadTypes() {
  ...
  if (missing.length)
    throw new Error(
      "Modelli/blocchi mancanti su " + ENV + ": " + missing.join(", "),
    );
}
...
async function ensureStoryClasses() {
  ...
  for (const cat of CATEGORIES) {
    ...
    if (found) {
      storyClassId[cat.key] = found.id;
      await tryPublish(found.id);
      console.log(`  story_class "${cat.label}" ok`);
      continue;
    }
    if (!COMMIT) {
      console.log(`  [dry] story_class "${cat.label}" da creare`);
      continue;
    }
    const created = await to.items.create({
      item_type: T("story_class"),
      label: { it: cat.label, en: cat.label },
      sort: String(sort++),
    });
...
async function ensureTopic(labelIt, labelEn) {
  ...
  if (found) {
    topicCache.set(labelIt, found.id);
    return found.id;
  }
  if (!COMMIT) {
    topicCache.set(labelIt, "DRY");
    return "DRY";
  }
  const created = await to.items.create({
    item_type: T("story_topic"),
...
async function promoteStories(cat, underId) {
  ...
  if (!COMMIT) {
    srcs.forEach((s) =>
      console.log(`   [dry] story_item <- ${val(s.slug, "it")}`),
    );
    return;
  }
  for (const s of srcs) {
    ...
    const topicId = topicLabels
      ? await ensureTopic(topicLabels.it, topicLabels.en)
      : null;
    ...
    const payload = {
      ...
      story_type: "news",
      date_of_publication: dateOnly
        ? { it: dateOnly, en: dateOnly }
        : { it: null, en: null },
    ...
    let rec;
    if (existing) {
      rec = await to.items.update(existing.id, payload);
      process.stdout.write(`   ~ story_item ${slugIt}\n`);
    } else {
...
async function ensureProfileShell(under) {
  ...
  if (existing) {
    ...
    await tryPublish(existing.id);
    console.log(`  profilo page "sottosegretario" ok (${profileId})`);
    return;
  }
  if (!COMMIT) {
    console.log(`  [dry] profilo page "sottosegretario" da creare`);
    return;
  }
  const created = await to.items.create({
    item_type: T("page"),
...
async function ensureIndexPage(cat) {
  ...
  if (existing) {
    indexPageId[cat.key] = existing.id;
    await tryPublish(existing.id);
    console.log(`  index_page "${cat.slug}" ok`);
    return;
  }
  if (!COMMIT) {
    console.log(`  [dry] index_page "${cat.slug}" da creare`);
    return;
  }
  ...
  const tab = buildBlockRecord({
    ...
    filter_style: JSON.stringify(["pills"]),
    filter_title: "Filtra per argomento",
    label_for_all: "Tutti",
    news_page_tab_type: "story_item",
    story_type: "news",
    filter_story: storyClassId[cat.key] || null,
    element_per_page: PER_CATEGORY,
  });
  const feed = buildBlockRecord({
    item_type: T("catalogue_feed"),
...
async function updateProfileContent(under, subpages) {
  if (!COMMIT || !profileId) {
    console.log(
      `  [dry] contenuto profilo (hero+bio+${CATEGORIES.length} preview+cta)`,
    );
    return;
  }
  const content = { it: [], en: [] };
  for (const l of LOCALES) {
    ...
    arr.push(
      buildBlockRecord({
        ...
        title: val(under.title, l) || val(under.title, "it"),
        paragraph: val(under.subtitle, l) || val(under.subtitle, "it"),
        cta: null,
      }),
    );
    ...
    arr.push(
      buildBlockRecord({
        item_type: T("text_image"),
        variant: "default",
        heading: "h2",
        text: textBlock("Biografia", bio),
        image: null,
    ...
    for (const cat of CATEGORIES) {
      ...
      const storyTab = buildBlockRecord({
        item_type: T("story_tab"),
        title: cat.label,
        news: ids,
      });
      const cta = indexPageId[cat.key]
        ? buildBlockRecord({
            item_type: T("internal_link"),
            label: cat.ctaAll,
            link_to: indexPageId[cat.key],
          })
        : null;
      arr.push(
        buildBlockRecord({
          item_type: T("news_feed"),
          background_color: "default",
          title: cat.label,
          paragraph: `Gli ultimi contenuti del Sottosegretario: ${cat.label.toLowerCase()}.`,
          tabs: [storyTab],
          cta,
        }),
      );
    ...
    if (nonEmpty(contatti)) {
      arr.push(
        buildBlockRecord({
          item_type: T("text_only"),
          background_color: "default",
          heading: "h2",
          text: textBlock("Contatti", contatti),
        }),
      );
    }
    content[l] = arr;
...
async function ensureDeleghe(subpages) {
  ...
  if (!COMMIT) {
    console.log(`  [dry] page "deleghe" ${existing ? "(update)" : "(create)"}`);
    return;
  }
  const content = { it: [], en: [] };
  for (const l of LOCALES) {
    const sections =
      subpages.delegheSections?.[l] || subpages.delegheSections?.it || [];
    content[l] = sections.length
      ? sections.map((sec, i) =>
          buildBlockRecord({
            ...
            background_color: "default",
            heading: "h2",
            text: textBlock(sec.title || `Deleghe — parte ${i + 1}`, sec.body),
          }),
        )
      : [
          buildBlockRecord({
            ...
            background_color: "default",
            heading: "h2",
            text: textBlock("Deleghe", "Contenuto in aggiornamento."),
          }),
        ];
  }
  let id;
  if (existing) {
    ...
    const created = await to.items.create({
      item_type: T("page"),
      title: { it: "Deleghe", en: "Delegations" },
      slug: { it: "deleghe", en: "delegations" },
      content,
    });
    id = created.id;
  ...
  try {
    await to.items.update(id, { parent_id: profileId });
    console.log(`    parent -> profilo`);
  } catch (e) {
    console.log(
      `    (parent albero non impostato: ${String(e.message).slice(0, 50)}; resta a root)`,
    );
  }
  await tryPublish(id);
}
...
async function loadSubpages() {
  ...
  for await (const s of from.items.listPagedIterator({
    filter: { type: "undersecretary_subpage" },
    nested: true,
    perPage: 50,
  })) {
    ...
    for (const l of LOCALES) {
      const arr = blocksOf(
        s.content_blocks?.[l] ?? (l === "it" ? s.content_blocks : null),
      );
      const texts = arr
        .filter((b) => b.relationships?.item_type?.data?.id === bodyId)
        .map((b) => ({
          title: b.attributes?.text_title || "",
          body: b.attributes?.body_text || "",
        }));
      if (slug === "biografia")
        out.biografia[l] = texts
          .map((t) => t.body)
          .filter(Boolean)
          .join("\n\n");
      if (slug === "contatti")
        out.contatti[l] = texts
          .map((t) => t.body)
          .filter(Boolean)
          .join("\n\n");
...
async function main() {
  console.log(
    `Build pagine Sottosegretario -> ${ENV}  [${COMMIT ? "COMMIT" : "DRY-RUN"}]`,
  );
  await loadTypes();
  const under = (
  ...
  console.log(
    `  bio it:${subpages.biografia.it ? "Y" : "-"} | deleghe sez it:${(subpages.delegheSections.it || []).length} | contatti it:${subpages.contatti.it ? "Y" : "-"}`,
  );
